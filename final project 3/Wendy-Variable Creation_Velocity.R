...
data = fread('applications_14_variables.csv',stringsAsFactors = F)
data$date = ymd(data$date)
data2 = fread('applications_14_variables.csv',stringsAsFactors = F)
data2$date = ymd(data2$date)
setDT(data2)
##rm(data2)

##Create Time Window
time_window = function(data2, n, byVar){
  dt1 = data2
...
for (i in c(0,1,3,7,14,30,60,90,180)){
    assign(paste0("data",'ssn',i), time_window(data2, i, 'ssn'))
}
assign(paste0('data','ssn',60),time_window(data2,60,'ssn'))
...
assign(paste0('data','homephone',7),time_window(data2,7,'homephone'))
assign(paste0('data','homephone',14),time_window(data2,14,'homephone'))
...
assign(paste0('data','dob_str','homephone',14),time_window(data2,14,c('dob_str','homephone')))
...
#################################################################################
write.csv(data, file = "homephone+dob_str.csv")
